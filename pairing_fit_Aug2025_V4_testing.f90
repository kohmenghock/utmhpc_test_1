...
!       keep_strength_dddi(:) =  model%strength(:)
       keep_strength_dddi(1) = -350.0
       keep_strength_dddi(2) = -350.0
...
!
   ...
   do isospin = 1,2
         ...
         do i = 1, HO_basis_size
            ...
            Epair_sen(isospin) = Epair_sen(isospin)+ave_kHFB(i)
         end do      
 ...
 !           fi=f((abs(sp_states(i,isospin)%energy-Fermi_energy(isospin)-0.5_rk*center)-width)/mu)
            Epair_dddi = Epair_dddi + ave_kHFB(i)*ave_pair_gap(i) !* fi
...
!         !
!         ! Divide Vpair by the old pairing strength
!         !
!         nucleus%pairing(isospin)%Vpair(:,:) = nucleus%pairing(isospin)%Vpair(:,:) / model%strength(isospin)
...
!      allocate(ave_kHFB(HO_basis%size))
      ...
      !
      k_HFB(:) = 0.0
!      call evaluate_k_HFB(nucleus,model,sp_states,HO_basis,isospin,Fermi_energy(isospin),&
!             ener_dens,array_size,delta_ener,epsilon,k_HFB)
   ...
   write(out_unit,*) 'Average Epair_cond dddi = ', Epair_dddi(1), Epair_dddi(2)
...
!=======================================================================================
   subroutine ave_density_discrete(nucleus,model,HO_basis,sp_states,isospin,gamma,order,dens,ener_dens,&
		array,delta_ener,ave_dens,epsilon,e_fermi,return_fermi)
   ...
   do i = 1, HO_basis%size
      ...
      dens(:) = 0.0
      do j = 1, array	!HO_basis%size-1

		if (sp_states(j,isospin)%omega<0) cycle
   ...
   if (return_fermi) then
      ...
      do i = 1, array
         ...
         if (((sum_particle - real(particle_num)) .gt. epsilon) .and. (i .gt. 1)) then
			...
			e_fermi = 0.5*(sp_states(i,isospin)%energy+sp_states(i+1,isospin)%energy)
            exit
         end if
      ...
      !
      dens_fermi = 0.0
      do i =1, HO_basis%size-1
         if ((e_fermi .gt. sp_states(i,isospin)%energy) .and. (e_fermi .lt. sp_states(i+1,isospin)%energy) ) then
            ratio = (e_fermi - sp_states(i,isospin)%energy) / &
			          (sp_states(i+1,isospin)%energy - sp_states(i,isospin)%energy)
...
!   real, dimension(HO_basis%size)::	dens
   ...
   if (return_fermi) then
      ...
      do i = 1, array
         sum_particle = sum_particle + ave_dens(i)*delta_ener
         !if (isospin .eq. 1) write(556,*) isospin, ener_dens(i), ave_dens(i), sum_particle
...
!=======================================================================================
   subroutine evaluate_g_HFB(nucleus,sp_states,HO_basis,isospin,ener,array,delta_ener,epsilon,g_HFB)
    ...
    real(kind=rk),dimension(:),allocatable	:: g_HFB,ener,dummy_g
   ...
   do i = 1, array		!HO_basis%size
      ...
      do iHF = 1, HO_basis%size
         ...
         sum_g = sum_g + ((nucleus%pairing(isospin)%gap(iHF))**2.0 / &
                 (2.0*((ener(i) - sp_states(iHF,isospin)%energy)**2.0 + (nucleus%pairing(isospin)%gap(iHF))**2.0)**1.5))
