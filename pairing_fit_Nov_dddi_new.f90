...
!    integer			:: count
    ...
    real(kind=rk),dimension(1:2)	:: Epair_sen, Fermi_energy, Epair_sen_2
    real(kind=rk),dimension(1:2)	:: V_old, V_new
    real(kind=rk)		:: ratio
    logical			:: return_fermi, seniority_loop
...
!
   ...
   if (model%pairing .eq. 'DDDI') then
      ...
      do while ( (abs(V_new(1) - V_old(1)) .gt. 0.1) .or. (abs(V_new(2) - V_old(2)) .gt. 0.1) )
         ...
         do isospin = 1, 2
            ...
            do while (abs(pme_new(isospin) - pme_old(isospin)) .gt. 0.001)
               ...
               ! allocate value for constant pairing matrix element
               pme_old(isospin) = pme_new(isospin)
               ...
               elseif (isospin .eq. 2) then
                  ...
                  pair_gap = 4.8 * (0.0181*abs(Epair_dddi_exact)*0.5 + 0.781) / real(Npart(2))**(1.0/3.0)
          
                  write(out_unit,*) 'Reduced Moller Nix gap for protons (DDDI) = ', pair_gap, &
                                ' with Epair_dddi exact = ', Epair_dddi_exact*0.50
                  write(out_unit,*)
               end if
               ...
               pme_new(isospin) = pairing_matrix
...
!               model%strength(isospin) = model%strength(isospin) *  Epair_sen(isospin) / Epair_dddi(isospin)
               ...
               write(out_unit,*)
               write(out_unit,*) '========================================================'
               write(out_unit,*) 'Estimation of delta force for isospin = ', isospin
...
!         write(out_unit,*) 
      ...
      end do !loop to adjust V0 strength
