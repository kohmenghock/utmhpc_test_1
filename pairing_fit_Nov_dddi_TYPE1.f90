...
!
    ...
    real(kind=rk)			:: gamma,e_fermi,integral,ener_min,ener_max
    integer, parameter   	:: order=2
!    integer			:: count
    real(kind=rk)		:: count
    real(kind=rk)		:: test_strength,test_strength_1,test_strength_2,Epair_dddi_exact
    real(kind=rk),dimension(1:2)	:: pme_old, pme_new, Epair_dddi
    real(kind=rk),dimension(1:2)	:: Epair_sen, Fermi_energy, Epair_sen_2, Epair_sen_3
    real(kind=rk),dimension(1:2)	:: V_old, V_new
    real(kind=rk)		:: ratio
    logical			:: return_fermi, seniority_loop, dddi_loop_total
    logical,dimension(1:2)	:: dddi_loop
    real(kind=rk)		:: ave_pair, sum_ave_khfb
    real(kind=rk)		:: mu, center, width, fj, fi
    real(kind=rk)		:: sum_tilde_k2, sum_ui2_vi2, ovlp, tmp, V0_new, V0_old
...
!
!  First check if fit_pairing is correctly chosen
!
...
!====================================================================================================================
!   
!    SENIORITY FORCE CASE
!
!====================================================================================================================


!
!   FIRST ROUND OF HF+BCS CALCULATIONS
!   
!
   ...
   if (model%pairing .eq. 'seniority') then
      ...
      write(out_unit,'(a85)') '===================================================================================='
      write(out_unit,'(a85)') '                                                                                    '
      write(out_unit,'(a85)') '             ENTERING UNIFORM GAP METHOD FOR PAIRING ESTIMATION                     '
      write(out_unit,'(a85)') '                                                                                    '
      write(out_unit,'(a85)') '===================================================================================='
      ...
      do while ((abs(pme_new(1) - pme_old(1)) .gt. 0.001) .or. (abs(pme_new(2) - pme_old(2)) .gt. 0.001))
         ...
         !
         !   Estimate pairing strengths for constant matrix element
         ! 
         ...
         do isospin = 1, 2
            ...
            elseif (isospin .eq. 2) then
                  ...
                  write(out_unit,*) 'Reduced Moller Nix gap for protons (DDDI) = ', pair_gap, &
                            ' with seniority pairing energy = ', nucleus%pairing(2)%pairing_energy
                  write(out_unit,*)
            end if
            ...
            !
            model%strength(isospin) = strength(isospin)
!            Gn_keep = strength(1)
!            Gp_keep = strength(2)
...
!            target_Q40 = nucleus%Q40(0)
      ...
      !
      do isospin = 1,2
         Epair_sen(isospin) = nucleus%pairing(isospin)%average_gap**2.0 / pme_new(isospin)
...
!
!   ENTERING ESTIMATION OF DELTA PAIRING INTENSITY
!   
!

   if (model%pairing .eq. 'DDDI') then
      ...
      !
      ! LOOP TO ESTIMATE V0 STRENGTHS
      !
      V_old(:) = model%strength(:) - 5.0_rk
      V_new(:) = V_old(:) + 5.0_rk
      dddi_loop_total = .true.
      dddi_loop(:) = .true.
      numerical_parameters%convergence%Nitermax = 300
      ...
      write(out_unit,*)
      write(out_unit,*) '==================================================================='
      write(out_unit,*) ' HFBCS: Vn = ', model%strength(1), ' Vp = ', model%strength(2)
      write(out_unit,*) '==================================================================='
...
!      do while ( (abs(V_new(1) - V_old(1)) .gt. 0.1 ) .and. (abs(V_new(2) - V_old(2)) .gt. 0.1 ) )
      do while (dddi_loop_total) 

         write(out_unit,*)
         write(out_unit,*)
         write(out_unit,*) '==================================================================='
         write(out_unit,*) '             BEGIN LOOP TO ESTIMATE DELTA FORCE                    '
         write(out_unit,*) '==================================================================='


         !
         ! ESTIMATION OF DELTA PAIRING INTENSITY
         !

         do isospin = 1, 2

            V_old(isospin) = V_new(isospin) 
            ...
            !======================================================
            ! ESTIMATE AVE PAIR CONDENSATION ENERGY FOR SENIORITY
            !======================================================
            ...
            !
            ! DO LOOP FOR CONSTANT PAIRING MATRIX ELEMENTS
            !

            do while (abs(pme_new(isospin) - pme_old(isospin)) .gt. 0.001)
               ...
               if (isospin .eq. 1) then
                  pair_gap = 4.80/real(Npart(1))**(1.0/3.0)
               elseif (isospin .eq. 2) then
                  Epair_dddi_exact = 0.0
                  do i = 1, HO_basis%size
                     if (sp_states(i,isospin)%energy .lt. (e_fermi-model%truncmin)) cycle
                     if (sp_states(i,isospin)%energy .gt. (e_fermi+model%truncmax)) exit
                     Epair_dddi_exact = Epair_dddi_exact + 0.50_rk * nucleus%pairing(isospin)%gap(i)**2.0 / &
                             sqrt((sp_states(i,isospin)%energy - e_fermi)**2.0 + (nucleus%pairing(isospin)%gap(i))**2.0)
                  end do
                  
                  pair_gap = 4.8 * (0.0181*abs(Epair_dddi_exact) + 0.781) / real(Npart(2))**(1.0/3.0)
          
                  write(out_unit,*) 'Reduced Moller Nix gap for protons (DDDI) = ', pair_gap, &
                                ' with Epair_dddi exact = ', Epair_dddi_exact
                  write(out_unit,*)
               end if
               ...
               !
               !   Only 1 iteration of HF+BCS equiv. to only BCS calculations (change to seniority calculation)
               !
               numerical_parameters%convergence%Nitermax = 1
...
!                    print_header_message=.false.)
               ...
               !=========================================================
               ! CALCULATE AVE PAIR CONDENSATION ENERGY FOR SENIORITY
               !=========================================================
...
!               Epair_sen(isospin) = nucleus%pairing(isospin)%average_gap**2.0 / pme_new(isospin) 
               ...
               Epair_sen_2(isospin) = 0.0
               Epair_sen_3(isospin) = 0.0
               ...
               do i = 1, HO_basis%size
                  ...
                  Epair_sen(isospin) = Epair_sen(isospin) + ave_kHFB(i)*nucleus%pairing(isospin)%gap(i)
                  
                  Epair_sen_2(isospin) = Epair_sen_2(isospin) + 0.50_rk * nucleus%pairing(isospin)%gap(i)**2.0 / &
                          sqrt((sp_states(i,isospin)%energy - Fermi_energy(isospin))**2.0 + (nucleus%pairing(isospin)%gap(i))**2.0)
               end do
               Epair_sen_3(isospin) = nucleus%pairing(isospin)%average_gap**2.0 / pme_new(isospin) 

               
               write(out_unit,'(a49,i1,a4,f8.3,a4)') 'Strutinsky averaging: Epair_cond seniority V1 (',isospin,')= ', &
                       Epair_sen(isospin),' MeV'

               write(out_unit,'(a49,i1,a4,f8.3,a4)') 'Strutinsky averaging: Epair_cond seniority V2 (',isospin,')= ', &
                       Epair_sen_2(isospin),' MeV'

               write(out_unit,'(a49,i1,a4,f8.3,a4)') 'Strutinsky averaging: Epair_cond seniority V3 (',isospin,')= ', &
                       Epair_sen_3(isospin),' MeV'
                       

               Epair_sen(isospin) = Epair_sen_3(isospin)
               
            end do ! while loop for constant matrix element
            
            !
            ! Change back to DDDI
            !
            model%pairing = 'DDDI'
            model%strength(isospin) = keep_strength_dddi(isospin)

            !
            !   COMPARE THE EXACT PAIR CONDENSATION ENERGY FROM SENIORITY OBTAINED HERE 
            !   WITH THE INITIAL PAIR CONDENSATION FROM THE DELTA SOLUTION AT THE TOP
            !   AND ADJUST THE PAIRING STRENGTHS
            !
            do while (abs(Epair_sen(isospin)-Epair_dddi(isospin)) .gt. 0.1)

               model%strength(isospin) = model%strength(isospin) * (1.0 + 0.05*(Epair_sen(isospin) - Epair_dddi(isospin)))
!               model%strength(isospin) = model%strength(isospin) *  Epair_sen(isospin) / Epair_dddi(isospin)

!               write(out_unit,*)
!               write(out_unit,*)
!               write(out_unit,*) '========================================================'
!               write(out_unit,*) 'Strutinsky averaging isospin: Estimation of delta force for isospin = ', isospin
!               write(out_unit,*) 'Strutinsky averaging isospin: ', isospin, ' Epair_conden (SEN) = ', Epair_sen(isospin)
!               write(out_unit,*) 'Strutinsky averaging isospin: ', isospin, ' Epair_conden (DDDI) = ', Epair_dddi(isospin)
!               write(out_unit,*) 'Strutinsky averaging isospin: ', isospin, ' V0_old = ', V_old(isospin)
!!               write(out_unit,*) 'Strutinsky averaging isospin: ', isospin, ' V0_new = ', V_new(isospin)
            ...
            end do !loop for adjusting Epair_dddi

            V_new(isospin) = model%strength(isospin)
            ...
            write(out_unit,*) 'Strutinsky averaging isospin: ', isospin, ' Epair_dddi = ', Epair_dddi(isospin)
         ...
         write(out_unit,*)
         write(out_unit,*) '================================================================================='
         write(out_unit,*) 'Strutinsky averaging: V0 = ', model%strength(1), model%strength(2)
         write(out_unit,*) 'Strutinsky averaging: Q20 = ', nucleus%Q20(0), ' Q40 = ', nucleus%Q40(0)
         write(out_unit,*) '================================================================================='
         write(out_unit,*)
         write(out_unit,*)
         ...
         do isospin = 1, 2
            ...
            write(out_unit,*) 
            write(out_unit,*) 'Strutinsky averaging: keep strength isospin ', isospin, ' = ', keep_strength_dddi(isospin)
            write(out_unit,*) 'Strutinsky averaging: new strength isospin ', isospin, ' = ', model%strength(isospin)
            write(out_unit,*)
                        
            if (abs(keep_strength_dddi(isospin) - V_new(isospin)) .lt. 1.0) then
               dddi_loop(isospin) = .false.	!false to end the loop
            else
               dddi_loop(isospin) = .true.
            end if
         end do
         
         if ((dddi_loop(1) .eqv. .false.) .and. (dddi_loop(2) .eqv. .false.)) then
            dddi_loop_total = .false.
         elseif ((dddi_loop(1) .eqv. .true.) .and. (dddi_loop(2) .eqv. .false.)) then
            dddi_loop_total = .true.
         elseif ((dddi_loop(1) .eqv. .false.) .and. (dddi_loop(2) .eqv. .true.)) then
            dddi_loop_total = .true.
         elseif ((dddi_loop(1) .eqv. .true.) .and. (dddi_loop(2) .eqv. .true.)) then
            dddi_loop_total = .true.
         end if


         !
         ! Update keep_strength_dddi
         !
         keep_strength_dddi(:) = model%strength(:)


         !
         ! Run HFBCS if V0 are not converged
         !
         if (dddi_loop_total) then
            write(out_unit,*)
            write(out_unit,*)
            write(out_unit,*) '==================================================================='
            write(out_unit,*) ' HFBCS: Vn = ', model%strength(1), ' Vp = ', model%strength(2)
      ...
      end do ! OUTER LOOP to adjust V0 strength
   ...
   !=============================================================================================================
   !   END OF PAIRING STRENGTH ESTIMATION
   !=============================================================================================================
